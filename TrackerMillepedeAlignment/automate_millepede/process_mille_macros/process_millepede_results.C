...
void process_millepede_results(std::string pedefilename = "/sphenix/tg/tg01/hf/rboucher43/eval_output/millepede.res",
  std::string newalignmentfilename = "/sphenix/tg/tg01/hf/rboucher43/eval_output/new_alignment_corrections.txt",
			       bool helical_fitter = true)
{
 ...
 int layer_keep = 0;
  ...
  // Also, can have layers fixed in one MVTX clamshell only.
  // Then only half of the expected staves will be present in the file.
  ...
  if (it3 != layer_stave_map.end())
    { 
      // the INTT is present
      auto it4 = layer_stave_map.find(4);
